...
- **优化**：移除强制添加平台前缀的逻辑，支持纯数字 ID。
- **修复**：重构推送逻辑，增加“寻找在线 Bot 直发”的降级方案，确保在各种 Koishi 环境下都能成功推送。

### 1.2.0

- **修复**：重构了推送逻辑的异常捕获，解决部分 Notifier 服务实现不规范导致的 `TypeError` 问题。
- **优化**：针对 OneBot 平台增加了 `target` 自动补全功能，当识别到纯数字目标 ID 时会自动添加 `onebot:` 前缀。
